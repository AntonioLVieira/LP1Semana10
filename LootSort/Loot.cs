...
namespace LootSort
{
    ...
    public class Loot : IComparable<Loot>
    {
        ...
        public int CompareTo(Loot otherLoot)
        {
            if (otherLoot == null)
            {
                return 1;
            }
            int comp = Kind.ToString().CompareTo(otherLoot.Kind.ToString());
            if (comp == 0)
            {
                comp = Value.ToString().CompareTo(otherLoot.Value.ToString());
            }
            if (comp == 0)
            {
                comp = Description.ToString().CompareTo(otherLoot.Description.ToString());
            }
            return comp;
        }
